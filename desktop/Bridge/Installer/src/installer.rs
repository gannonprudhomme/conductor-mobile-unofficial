...
impl ConductorPaths {
    fn new() -> Result<Self, String> {
        Self::with_source_bridge_dir(default_source_bridge_dir()?)
    }

    fn with_source_bridge_dir(source_bridge_dir: PathBuf) -> Result<Self, String> {
        let home =
...
fn default_source_bridge_dir() -> Result<PathBuf, String> {
    let bridge_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(Path::to_path_buf)
        .ok_or("Could not resolve bridge root")?;

    Ok(bridge_root.join("Proxy"))
}

pub fn install_bridge_from(source_bridge_dir: PathBuf) -> Result<(), String> {
    install_bridge_with_paths(ConductorPaths::with_source_bridge_dir(source_bridge_dir)?)
}

fn install_bridge_with_paths(conductor_paths: ConductorPaths) -> Result<(), String> {
...
pub fn get_bridge_status_with_client(
