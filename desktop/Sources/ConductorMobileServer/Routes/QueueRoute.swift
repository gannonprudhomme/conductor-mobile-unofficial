...
enum QueueRoute {
    static func delete(
        ...
        try await performMutation(
            commandTimeout: persistenceTimeout,
            ...
            try await uiHook.deleteQueuedMessage(
                ...
                waitUntilChangeAvailableInDatabase: {
                    try await waitUntilQueuedMessageIsCancelled(
                        sessionID: sessionID,
                        messageID: messageID,
                        database: database,
    ...
    private static func waitUntilQueuedMessageIsCancelled(
        sessionID: Session.ID,
        messageID: Message.ID,
        database: any DatabaseReader,
        timeout: Duration
    ...
    private static func performMutation(
        ...
        do {
            ...
            case .commandFailed(let message):
                throw PlainTextResponseError(.conflict, message: message)
            case .deliveryUnknown:
                throw PlainTextResponseError(
                    .serviceUnavailable,
                    message: deliveryUnknownMessage
