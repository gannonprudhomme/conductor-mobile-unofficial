...
struct ServerTests {
    @Test("Transcript benchmark reports desktop pipeline stages")
    func transcriptBenchmarkStages() async throws {
        let environment = ProcessInfo.processInfo.environment
        guard let databasePath = environment["TRANSCRIPT_BENCHMARK_DATABASE"],
              let workloadJSON = environment["TRANSCRIPT_BENCHMARK_WORKLOADS"] else {
            return
        }
        let workloads = try JSONDecoder().decode(
            [TranscriptBenchmarkWorkload].self,
        ...
        let database = try ConductorDatabase.open(
            at: URL(filePath: databasePath)
        )
        ...
        for workload in workloads {
            for iteration in 0..<(warmupCount + iterationCount) {
                ...
                let messages = try await database.read { database in
                    try Message.all(
                        forWorkspaceID: workload.workspaceID,
                        sessionID: workload.sessionID
                    )
                    .fetchAll(database)
                }
                let queryMilliseconds = milliseconds(from: queryStart, to: clock.now)

                let partitionStart = clock.now
                let history = messages
                    .filter { !$0.isQueued }
                    .sorted(by: isEarlierInBenchmarkHistory)
                let queue = messages
                    .filter(\.isQueued)
                    .sorted(by: isEarlierInBenchmarkQueue)
                let event = MessageSyncEvent.snapshot(
                    history,
                    cursor: history.last?.id,
                    queuedMessages: queue
                )
                let partitionMilliseconds = milliseconds(
                    from: partitionStart,
                    to: clock.now
                )

                let encodingStart = clock.now
                let encoded = try JSONEncoder.conductor.encode(event)
                let encodingMilliseconds = milliseconds(
                    from: encodingStart,
                    to: clock.now
                )
    ...
    @Test("Transcript benchmark serves the real database copy")
    func transcriptBenchmarkServer() async throws {
        let environment = ProcessInfo.processInfo.environment
        guard environment["TRANSCRIPT_BENCHMARK_SERVE"] == "1",
              let databasePath = environment["TRANSCRIPT_BENCHMARK_DATABASE"] else {
            return
        }
        let port = Int(environment["TRANSCRIPT_BENCHMARK_PORT"] ?? "") ?? 3_768
        let database = try ConductorDatabase.open(
            at: URL(filePath: databasePath)
        )
        try await Server.makeApplication(
            database: database,
            port: port
        )
        .run()
...
private struct TranscriptBenchmarkWorkload: Decodable {
    let label: String
...
private struct TranscriptDesktopBenchmarkResult: Encodable {
    let label: String
    let iteration: Int
    let messageCount: Int
    let encodedBytes: Int
    let queryMilliseconds: Double
    let partitionMilliseconds: Double
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case queryMilliseconds = "query_ms"
        case partitionMilliseconds = "partition_ms"
        case encodingMilliseconds = "encoding_ms"
    }
}
...
private func isEarlierInBenchmarkHistory(
    _ lhs: Message,
    _ rhs: Message
) -> Bool {
...
private func isEarlierInBenchmarkQueue(
    _ lhs: Message,
    _ rhs: Message
) -> Bool {
