...
struct QueueRouteTests {
    ...
    @Test("Naturally consuming a queued message does not confirm its deletion")
    func consumedMessageIsNotDeleted() async throws {
        let (database, workspace, session, _) = try await queueRouteDatabase()
        var uiHook = WorkspaceUIHook.liveValue
        uiHook.deleteQueuedMessage = {
            _,
        ...
        let message = try await database.read { database in
            try Message.find("message-1").fetchOne(database)
        }
        #expect(message?.sentAt != nil)
        #expect(message?.cancelledAt == nil)
    }
