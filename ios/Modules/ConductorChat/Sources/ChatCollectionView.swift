    ...
    static func shouldAnimateBottomFollow(
        ...
        let hasDisclosureChange = rows.contains { row in
            guard case .turnSummary(let summary) = row.content,
                  let wasExpanded = previousSummaries[summary.id]
            else {
                return false
            }
    ...
    final class Coordinator: NSObject, UICollectionViewDelegate, UIGestureRecognizerDelegate {
        ...
        func render(
            ...
            dataSource.apply(
                ...
                guard let self,
                      let collectionView,
                      snapshotApplicationState.complete(generation)
                else {
                    return
                }
        ...
        @objc private func expandedSummaryTargetTapped(
            ...
            for indexPath in collectionView.indexPathsForVisibleItems {
                guard let itemID = dataSource.itemIdentifier(for: indexPath),
                      ...
                      case let .turnSummary(summary) = row.content,
                ...
                // Ignore the cell itself; accept only the deliberately expanded area immediately
                // above or below it.
                guard !rowFrame.contains(location),
                      rowFrame.insetBy(
                          dx: 0,
        ...
        private func correctScrollPosition(in collectionView: UICollectionView) {
            ...
            if scrollPolicy.isFollowingBottom, scrollPolicy.hasDisplayedContent {
                ...
                if collectionView.isScrollAnimating,
                   !shouldAnimateNextBottomCorrection {
                    return
                }
        ...
        private func setContentOffsetY(
            ...
            if animated {
                ...
                stopNativeScrollAnimation(in: collectionView)
                performCoordinatorOffsetChange(setContentOffset)
            }
        }
    }
}
