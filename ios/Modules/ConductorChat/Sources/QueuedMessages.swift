...
struct QueuedMessagesView: View {
    ...
    var body: some View {
        ...
        .glassEffect(
            .clear.tint(.theme(.background).opacity(0.75)),
    ...
    private var messageListHeight: CGFloat {
        let mainHeight = QueuedMessagesPresentation.height(
            isExpanded: true,
            rowHeight: rowHeight,
            numRows: store.displayedMessages.count
        )
        
         return mainHeight - QueuedMessagesPresentation.headerHeight - QueuedMessagesPresentation.separatorHeight
    }
...
private struct QueuedMessageRow: View {
    ...
    private var row: some View {
        HStack(spacing: 16) {
            ...
            LucideIcon(Lucide.gripVertical, style: .body)
                .foregroundStyle(.theme(.sidebarMutedForeground))
//                .frame(width: 44, height: 44)
                .contentShape(.rect)
