...
struct QueuedMessagesTests {
    ...
    @Test("An expanded queue has a fixed four-row height")
    func queueHeight() async throws {
        let longSession = try makeSession()
        let shortSession = try makeSession(id: "session-2")
        ...
        try await withDependencies {
            ...
            let keyboardConstrainedSize = longHostingController.sizeThatFits(
                in: CGSize(width: 390, height: 400)
            )
            #expect(abs(keyboardConstrainedSize.height - expandedHeight) < 1)
            ...
            #expect(abs(fittedSize.height - expandedHeight) < 1)
    ...
    func resumeQueuedMessages() async throws {
        var session = try makeSession(status: "working")
        session.queuePausedAt = "2026-07-19T12:00:00Z"
