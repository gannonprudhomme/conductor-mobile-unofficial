...
struct CloudCredentialClientTests {
    @Test("The injectable credential boundary supports save, load, replace, and delete")
    func mockCredentialStorage() async throws {
        ...
        try await credentials.saveAPIKey(apiKey: "replacement-api-key")
        #expect(try await credentials.loadAPIKey() == "replacement-api-key")
